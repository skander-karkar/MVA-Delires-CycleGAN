...
def generator(input_gen, name = "generator", skip = False): # 9 resnet blocks
	with tf.variable_scope(name):
		f = 7
		ks = 3
		padding = "REFLECT"
...
class CycleGAN:
	...
	def __init__(self, dataset_name, lambda_A = 10, lambda_B = 10, lambda_I = 5, base_lr = 0.0002, do_flipping = True, skip = SKIP):
		...
		self._skip = skip # use skip connection over entire generator
	...
	def create_model(self):
		...
		with tf.variable_scope("model") as scope:
			...
			self.fake_images_B = generator(self.input_A, name = "g_A", skip = self._skip)
			self.fake_images_A = generator(self.input_B, name = "g_B", skip = self._skip)

			scope.reuse_variables()

			self.wrong_fake_B = generator(self.input_A, name = "g_B", skip = self._skip)
			self.wrong_fake_A = generator(self.input_B, name = "g_A", skip = self._skip)
			...
			self.cycle_images_A = generator(self.fake_images_B, "g_B", skip = self._skip)
			self.cycle_images_B = generator(self.fake_images_A, "g_A", skip = self._skip)
